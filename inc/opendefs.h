...
enum {
   ...
   ERR_FREEING_UNUSED                  = 0x2d, // freeing unused memory  (code location  {0})
   ERR_FREEING_ERROR                   = 0x2e, // freeing memory unsupported memory  (code location  {0})
   ...
   ERR_OPENSERIAL_BUFFER_OVERFLOW      = 0x43, // buffer overflow for output in openserial (indexWrite {0}, location {1})
...
typedef struct{
   ...
   uint8_t        etx;                          // the total path quality to the bottleneck
} btneck_t;
END_PACK
